...
NOISE_FILE_NAMES is shared: both StateView and critic evidence exclude the same
well-known cache/config file names.
"""

# Shared by StateView exclusion and critic evidence exclusion.
NOISE_FILE_NAMES: frozenset[str] = frozenset({"CACHEDIR.TAG", "pyvenv.cfg"})
...
STATE_VIEW_NOISE_DIRS: frozenset[str] = frozenset({"__pycache__", "node_modules", "dist", "build"})
STATE_VIEW_NOISE_SUFFIXES: frozenset[str] = frozenset({".pyc", ".pyo", ".lock", ".egg-info"})

# Generated artifact exclusion — which tracked files are restored before a
# worker branch is merged into main. Narrower than StateView noise: only
...
# Critic evidence noise — untracked files excluded from the critic's worktree
# evidence window. Superset of generated artifact dirs, plus .git internals.
CRITIC_EVIDENCE_NOISE_DIRS: frozenset[str] = frozenset(
    {".git", "__pycache__", "node_modules", ".pytest_cache", ".ruff_cache", ".venv"}
)
CRITIC_EVIDENCE_NOISE_SUFFIXES: frozenset[str] = frozenset({".pyc", ".pyo", ".pyd", ".lock"})
