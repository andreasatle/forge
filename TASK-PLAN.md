...
- **State Ownership: MOSTLY COMPLETE**. Active workers use `build_read_registry`, build a `StateView`, and are instructed to produce `WorkOutput`; `StateService.apply_work_output` is the main mutation boundary. Remaining drift: write tools and blackboard-related vocabulary still exist as stale surfaces, even if not active in workers.
