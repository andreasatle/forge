...
**Architecture Audit**
- **Task Lifecycle: PARTIAL**. Runtime has `PENDING/RUNNING/INTEGRATED/FAILED/CANCELLED`; readiness is computed, not stored as a state. Accepted worker output is now integrated through `StateService.apply_work_output`; remaining lifecycle drift is that integration outcome handling is still coupled to scheduler state transitions. See [models.py](/Users/andreasatle/Projects/Forge/src/forge/core/models.py:38), [models.py](/Users/andreasatle/Projects/Forge/src/forge/core/models.py:245), [scheduler.py](/Users/andreasatle/Projects/Forge/src/forge/core/scheduler.py:94).
...
- **State Ownership: MOSTLY COMPLETE**. Active workers use `build_read_registry`, build a `StateView`, and are instructed to produce `WorkOutput`; `StateService.apply_work_output` is the main mutation boundary. Remaining drift: retained mutating tool APIs still exist as stale surfaces.
...
5. Remove or quarantine obsolete mutating tool API abstractions.
