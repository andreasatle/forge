"""Tests for the integrator — apply, test failure reporting, and response shape."""
...
async def test_apply_delta_called_with_provided_delta():
    """integrate calls state_service.apply_delta with the exact delta passed in."""
    ss = _mock_ss()
...
async def test_run_tests_called_after_successful_apply():
    """integrate calls state_service.run_tests after apply_delta succeeds."""
    ss = _mock_ss()
    await integrate(request_id=uuid4(), state_service=ss, delta=DeltaState())
...
async def test_returns_failed_when_apply_raises():
    """integrate returns FAILED status when apply_delta raises."""
    ss = _mock_ss()
...
async def test_returns_failed_when_tests_fail():
    """integrate returns FAILED status when run_tests returns passed=False."""
    ss = _mock_ss(passed=False, failures=["FAILED tests/test_foo.py::test_x"], summary="1 failed")
    response = await integrate(request_id=uuid4(), state_service=ss, delta=DeltaState())
    assert response.status == ResponseStatus.FAILED
